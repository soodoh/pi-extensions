...
import type { PromptContextBuilder } from "./prompt-context-builder";
import type { TranscriptPromptContextBuilder } from "./transcript-prompt-context-builder";
...
function normalizeSuggestion(value: string, maxChars: number): string {
	...
	const trimmedTrailing = normalizedLineEndings
		...
		.replace(/\n{3,}/g, "\n\n")
		.trim();
	return trimmedTrailing.length > maxChars
		? trimmedTrailing.slice(0, maxChars).trimEnd()
		: trimmedTrailing;
}
...
export class SuggestionEngine {
	...
	public async suggest(
		...
		if (
			config.suggestion.fastPathContinueOnError &&
			turn.status !== "success"
		) {
			return {
				kind: "suggestion",
		...
		const raw = await this.generateWithBestAvailableStrategy(
			turn,
			seed,
			steering,
			settings,
	...
	private async generateWithBestAvailableStrategy(
		...
		if (
			...
			this.deps.transcriptPromptContextBuilder
		) {
			const sampledOut =
				config.suggestion.transcriptRolloutPercent < 100 &&
				stableSamplePercent(turn.turnId || turn.sourceLeafId) >=
					config.suggestion.transcriptRolloutPercent;
			if (sampledOut) {
				return await this.generateCompactSuggestion(
					turn,
					seed,
					steering,
					settings,
			...
			try {
				const transcriptContext =
					this.deps.transcriptPromptContextBuilder.build(
						seed,
						steering,
						config,
					);
				const overContextLimit =
					...
					transcriptContext.contextUsagePercent >
						config.suggestion.transcriptMaxContextPercent;
				if (overContextLimit) {
					return await this.generateCompactSuggestion(
						turn,
						seed,
				...
				const result = await this.deps.modelClient.generateSuggestion(
					transcriptContext,
					settings,
				);
				return {
	...
	private async generateCompactSuggestion(
		...
		const context = this.deps.promptContextBuilder.build(
			turn,
			seed,
			steering,
			config,
