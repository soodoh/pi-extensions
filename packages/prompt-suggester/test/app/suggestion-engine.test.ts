...
test("SuggestionEngine uses transcript-steering mode when eligible", async () => {
	...
	expect(result.metadata.strategy).toBe("transcript-steering");
	expect(result.metadata.requestedStrategy).toBe("transcript-steering");
...
test("SuggestionEngine falls back to compact mode when transcript guardrails reject the run", async () => {
	...
	expect(result.metadata.fallbackReason).toBe("transcript_context_limit");
...
test("SuggestionEngine falls back to compact mode when transcript rollout samples out", async () => {
	...
	expect(result.kind).toBe("suggestion");
	expect(result.metadata.strategy).toBe("compact");
	expect(result.metadata.sampledOut).toBe(true);
	expect(result.metadata.fallbackReason).toBe("transcript_rollout_skip");
	expect(calls.length).toBe(1);
});
